...
function c501000079.cfilter(c,tp)
	return c:IsRace(RACE_BEAST) and c:IsType(TYPE_PENDULUM) and c:IsAbleToRemoveAsCost()
		and Duel.IsExistingTarget(c501000079.filter,tp,LOCATION_MZONE,0,1,c) 
end
function c501000079.filter(c)
...
function c501000079.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.IsExistingMatchingCard(c501000079.cfilter,tp,LOCATION_MZONE,0,3,nil,tp) and ct>3 end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,c501000079.cfilter,tp,LOCATION_MZONE,0,3,3,nil,tp)
