...
function c511000358.cfilter(c)
	return c:IsRace(RACE_DRAGON)
end
function c511000358.condition(e,tp,eg,ep,ev,re,r,rp)
	local g=eg:Filter(c511000358.cfilter,nil,tp)
	return g
end
...
function c511000358.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>1
		and Duel.IsExistingMatchingCard(c511000358.filter,tp,LOCATION_DECK,0,2,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SET)
	local g=Duel.SelectTarget(tp,c511000358.filter,tp,LOCATION_DECK,0,2,2,nil)
end
