...
function c810000034.filter(c,e,tp)
	...
	elseif c:IsAttackBelow(500) then
		return Duel.IsPlayerCanSpecialSummonMonster(tp,c:GetCode(),0,c:GetType(),c:GetAttack(),c:GetDefence(),c:GetLevel(),c:GetRace(),c:GetAttribute()) 
...
function c810000034.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsCode(40640057) then
		...
		if ft<=0 
			or not Duel.IsPlayerCanSpecialSummonMonster(tp,tc:GetCode(),0,0x4011,tc:GetAttack(),tc:GetDefence(),tc:GetLevel(),
		...
		for i=1,2 do
			...
			--when leave field remove from the current game (same as token monsters)
			local e7=Effect.CreateEffect(e:GetHandler())
			e7:SetProperty(EFFECT_FLAG_UNCOPYABLE+EFFECT_FLAG_CANNOT_DISABLE)
			e7:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_CONTINUOUS)
			e7:SetCode(EVENT_LEAVE_FIELD)
			e7:SetOperation(c810000034.leaveop)
...
function c810000034.leaveop(e,tp,eg,ep,ev,re,r,rp)
