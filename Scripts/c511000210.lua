...
function c511000210.cfilter(c,e,tp)
	local gr=c:GetAttribute()
	return Duel.IsExistingMatchingCard(c511000210.spfilter,tp,0,LOCATION_DECK,1,nil,gr,e,tp)
end
function c511000210.spfilter(c,gr,e,tp)
	return c:IsAttribute(gr) and c:IsCanBeSpecialSummoned(e,0,1-tp,false,false,POS_FACEUP)
end
...
function c511000210.spfilter2(c,gr,e,tp)
	return c:IsAttribute(gr) and c:IsCanBeSpecialSummoned(e,0,1-tp,false,false,POS_FACEUP)
end
function c511000210.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local slv=tc:GetAttribute()
	local sg=Duel.GetMatchingGroup(c511000210.spfilter,tp,0,LOCATION_DECK,nil,slv,e,tp)
