...
function c810000054.initial_effect(c)
	...
	e1:SetTarget(c810000054.target)
...
function c810000054.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c810000054.descost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c810000054.costfilter,tp,LOCATION_GRAVE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,c810000054.costfilter,tp,LOCATION_GRAVE,0,1,1,nil)
	Duel.Remove(g,POS_FACEUP,REASON_COST)
	Duel.Remove(e:GetHandler(),POS_FACEUP,REASON_COST)
end
function c810000054.descon(e,c,tp)
	return Duel.IsExistingMatchingCard(Card.IsSetCard,tp,LOCATION_GRAVE,0,1,nil,0x33)
end
function c810000054.desfilter(c)
	return c:IsFaceup() and c:IsControler(1-tp) and c:IsDestructable()
end
function c810000054.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and c810000054.desfilter(chkc) end
