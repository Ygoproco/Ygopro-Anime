...
function c511000679.initial_effect(c)
	...
	e1:SetTarget(c511000679.pltg)
	e1:SetOperation(c511000679.plop)
	c:RegisterEffect(e1)
end
...
function c511000679.pltg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsSetCard,tp,LOCATION_HAND,0,1,nil,0x34) and Duel.GetLocationCount(tp,LOCATION_SZONE)>0 end
	Duel.SetOperationInfo(0,CATEGORY_LEAVE_HAND,nil,1,tp,LOCATION_HAND)
end
function c511000679.plop(e,tp,eg,ep,ev,re,r,rp)
	local ft=Duel.GetLocationCount(tp,LOCATION_SZONE)
	if ft<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOFIELD)
	local g=Duel.SelectMatchingCard(tp,Card.IsSetCard,tp,LOCATION_HAND,0,ft,ft,nil,0x34)
	if g:GetCount()>0 then
		...
		local d=Duel.GetMatchingGroupCount(Card.IsSetCard,tp,LOCATION_SZONE,0,nil,0x34)*500
