...
function c511000325.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local con2=Duel.IsPlayerCanSpecialSummonMonster(tp,29843092,0xf,0x4011,0,1000,2,RACE_BEAST,ATTRIBUTE_LIGHT,POS_FACEUP_DEFENCE,1-tp)
	local con3=Duel.GetLocationCount(tp,LOCATION_MZONE,tp)>2
	local con4=Duel.IsPlayerCanSpecialSummonMonster(tp,29843092,0xf,0x4011,0,1000,2,RACE_BEAST,ATTRIBUTE_LIGHT,POS_FACEUP_DEFENCE,tp)
...
function c511000325.spop(e,tp,eg,ep,ev,re,r,rp)
	if e:GetLabel()==0 then
		...
		if not Duel.IsPlayerCanSpecialSummonMonster(tp,29843092,0xf,0x4011,0,1000,2,RACE_BEAST,ATTRIBUTE_LIGHT,POS_FACEUP_DEFENCE,tp) then return end
		for i=1,3 do
			local token=Duel.CreateToken(tp,29843091+i)
			if Duel.SpecialSummonStep(token,0,tp,tp,false,false,POS_FACEUP_DEFENCE) then
