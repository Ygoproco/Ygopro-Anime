...
function c511001262.damop(e,tp,eg,ep,ev,re,r,rp)
	...
	if g1:GetCount()>0 then
		...
		c511001262[0]=c511001262[0]+sum
	end
	if g2:GetCount()>0 then
		local sum=g2:GetSum(Card.GetAttack)
		c511001262[1]=c511001262[1]+sum
	end
end
...
function c511001262.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return c511001262[0]>0 or c511001262[1]>0 end
	Duel.SetOperationInfo(0,CATEGORY_DAMAGE,nil,0,PLAYER_ALL,0)
end
function c511001262.activate(e,tp,eg,ep,ev,re,r,rp)
	if c511001262[0]>0 then
		Duel.Damage(tp,c511001262[0],REASON_EFFECT)
	end
	if c511001262[1]>0 then
		Duel.Damage(1-tp,c511001262[1],REASON_EFFECT)
	end
end
