...
function c100000350.initial_effect(c)
	...
	e3:SetOperation(c100000350.op)
	c:RegisterEffect(e3)
end
function c100000350.target(e,c)
	return c:IsAttribute(ATTRIBUTE_WATER) and c:IsFaceup()
end
function c100000350.filterx(c)
	return c:IsType(TYPE_XYZ) and c.xyz_count and c.xyz_count>0
end
function c100000350.op(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local g=Duel.GetMatchingGroup(c100000350.filterx,c:GetControler(),LOCATION_EXTRA,LOCATION_EXTRA,nil)
	local tc=g:GetFirst()
	while tc do
		...
		if tc:GetFlagEffect(419)==0 then
			...
			e1:SetCondition(c100000350.xyzcon)
			e1:SetOperation(c100000350.xyzop)
...
function c100000350.mfilter(c,rk,xyz)
	return c:IsFaceup() and xyz.xyz_filter(c) and c:IsCanBeXyzMaterial(xyz) and c:IsXyzLevel(xyz,rk)
end
function c100000350.amfilter(c)
	return c:GetEquipGroup():IsExists(Card.IsHasEffect,1,nil,511001175)
end
function c100000350.subfilter(c,rk,xyz,tck)
	return c:IsFaceup() and c.xyzsub and c.xyzsub==rk and not c:IsStatus(STATUS_DISABLED) 
		and xyz.xyz_filter(tck)
end
function c100000350.xyzcon(e,c,og)
	if c==nil then return true end
	local tp=c:GetControler()
	local rk=c:GetRank()
	local ct=c.xyz_count
	local mg=Duel.GetMatchingGroup(c100000350.mfilter,tp,LOCATION_MZONE,0,nil,rk,c)
	local tck=e:GetLabelObject()
	local mg2=Duel.GetMatchingGroup(c100000350.subfilter,tp,LOCATION_ONFIELD,0,nil,rk,c,tck)
	mg:Merge(mg2)
	if not mg:IsExists(c100000350.amfilter,1,nil) and not mg:IsExists(Card.IsHasEffect,1,nil,511001225) 
		and not mg:IsExists(c100000350.subfilter,1,nil,rk,c,tck) then return false end
	local g=mg:Filter(c100000350.amfilter,nil)
...
function c100000350.xyzop(e,tp,eg,ep,ev,re,r,rp,c,og)
	local c=e:GetHandler()
	local rk=c:GetRank()
	local ct=c.xyz_count
	local mg=Duel.GetMatchingGroup(c100000350.mfilter,tp,LOCATION_MZONE,0,nil,rk,c)
	...
	while ct>0 do
		...
		if g2:GetFirst() and c100000350.amfilter(g2:GetFirst()) then
